...
@Injectable()
export class GroqLLM extends BaseLLMService {
  ...
  async callStructured<T extends z.ZodTypeAny>(
    ...
    try {
      const apiKey = this.nestConfigService.get('GROQ_API_KEY');
      if (!apiKey) {
        throw new Error(
          `Groq API key (GROQ_API_KEY) is not configured. Cannot initialize ChatGroq model.`,
        );
      }
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Unknown error occurred while initializing Groq LLM';
      throw new Error(`GroqLLM provider error (${model}): ${message}`);
    }
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Unknown error occurred during Groq LLM structured call';
      throw new Error(`GroqLLM call failed for model "${model}": ${message}`);
    }
